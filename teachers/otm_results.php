<?php
// teachers/otm_results_view.php - View OTM results by class (teacher portal, read-only)
...
$tguard_fallback_path = '/teachers/otm_results_view.php';
...
try {
    ...
    if ($hasPupilsTable) {
        if ($restrictToOwnClass && !$scopeBroken) {
            $classSql = "
                SELECT p.class_code, COUNT(*) AS cnt
                FROM pupils p
            ";
            $classParams = [];
            if ($teacherClassId > 0) {
                $classSql .= " WHERE p.class_id = :scope_class_id";
                $classParams[':scope_class_id'] = $teacherClassId;
            } else {
                $classSql .= " WHERE p.class_code = :scope_class_code";
                $classParams[':scope_class_code'] = $teacherClassCode;
            }
            $classSql .= " AND p.class_code IS NOT NULL AND p.class_code <> '' GROUP BY p.class_code ORDER BY p.class_code ASC";
        ...
        if ($restrictToOwnClass && !$scopeBroken && $classCode === '' && count($classOptions) === 1) {
            $classCode = (string)$classOptions[0]['class_code'];
        }
    }
    ...
    error_log('[TEACHERS_OTM_RESULTS_VIEW] init failed: ' . $e->getMessage());
...
$loadReady = $schemaReady && !$scopeBroken && $classCode !== '' && $otmExamId > 0;

if ($loadRequested && !$loadReady) {
    if (!$schemaReady) $loadIssues[] = 'Schema/tables not ready';
    if ($scopeBroken) $loadIssues[] = 'Teacher class scope is missing';
...
if ($loadReady) {
    try {
        ...
        if ($q !== '') {
            $allSql .= " AND (p.surname LIKE :q OR p.name LIKE :q)";
            $allParams[':q'] = '%' . $q . '%';
        }
        ...
        $rows = [];
        error_log('[TEACHERS_OTM_RESULTS_VIEW] load failed: ' . $e->getMessage());
...
<style nonce="<?= h_attr($_SESSION['csp_nonce'] ?? '') ?>">
  ...
  .otmv-col-eq { width: 4.8rem; min-width: 4.8rem; max-width: 4.8rem; }
  .otmv-col-eq .otmv-head-wrap { display: inline-block; line-height: 1.05; }
      ...
      <div class="card-body">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
          <div>
            ...
            <div class="small text-muted">Teacher view for class OTM results by selected OTM exam.</div>
          </div>
          <div class="d-flex flex-wrap gap-2 small">
            <span class="otmv-chip"><i class="bi bi-shield-check"></i> View only</span>
            <?php if ($scopeLabel !== null): ?>
              <span class="otmv-chip"><i class="bi bi-filter-circle"></i> Scope: <?= h($scopeLabel) ?></span>
            <?php endif; ?>
            <?php if ($selectedExam): ?>
        ...
        <?php if ($scopeBroken): ?>
          <div class="alert alert-danger mb-3">
            Your teacher account is missing class scope. Contact administrator.
          </div>
        <?php endif; ?>
        ...
        <form method="get" class="row g-2 align-items-end">
          <div class="col-md-3 col-xl-2">
            ...
            <?php if ($restrictToOwnClass && !$scopeBroken): ?>
              <div class="form-text">Limited to your assigned class scope.</div>
            <?php endif; ?>
          </div>
            ...
            <table class="table table-sm table-bordered align-middle otmv-table">
              <thead class="table-light">
                <tr>
                  ...
                  <th class="text-center otmv-col-eq">Fan 1</th>
                  <th class="text-center otmv-col-eq">Fan 2</th>
                  <th class="text-center otmv-col-eq">Ona</th>
                  <th class="text-center otmv-col-eq"><span class="otmv-head-wrap">Matema-<br>tika</span></th>
                  <th class="text-center otmv-col-eq">Tarix</th>
              ...
              <tbody>
                <?php foreach ($rows as $i => $r): ?>
                  ...
                  <tr class="<?= h_attr($rowClass) ?>">
                    ...
                    <td class="otmv-num otmv-col-eq"><?= $r['major1_correct'] !== null ? (int)$r['major1_correct'] : '—' ?></td>
                    <td class="otmv-num otmv-col-eq"><?= $r['major2_correct'] !== null ? (int)$r['major2_correct'] : '—' ?></td>
                    <td class="otmv-num otmv-col-eq"><?= $r['mandatory_ona_tili_correct'] !== null ? (int)$r['mandatory_ona_tili_correct'] : '—' ?></td>
                    <td class="otmv-num otmv-col-eq"><?= $r['mandatory_matematika_correct'] !== null ? (int)$r['mandatory_matematika_correct'] : '—' ?></td>
                    <td class="otmv-num otmv-col-eq"><?= $r['mandatory_uzb_tarix_correct'] !== null ? (int)$r['mandatory_uzb_tarix_correct'] : '—' ?></td>
