...
// For term-to-term comparisons we sometimes need a scope without exam_id AND without term.
$whereNoExamNoTermByKey = $whereNoExamByKey;
$paramsNoExamNoTermByKey = $paramsNoExamByKey;
unset($whereNoExamNoTermByKey['term'], $paramsNoExamNoTermByKey['term']);
$whereNoExamNoTermSql = $whereNoExamNoTermByKey ? ('WHERE ' . implode(' AND ', array_values($whereNoExamNoTermByKey))) : '';
$paramsNoExamNoTerm = $paramsNoExamNoTermByKey;

// Determine if selected term is the latest term available in the current slice (ignoring term + exam_id).
$latestTermRow = fetch_one($pdo, "
    SELECT MAX(e.term) AS latest_term
    FROM results r
    JOIN pupils p   ON p.id = r.pupil_id
    JOIN subjects s ON s.id = r.subject_id
    JOIN exams e    ON e.id = r.exam_id
    $whereNoExamNoTermSql
", $paramsNoExamNoTerm);

$latestTermInSlice = !empty($latestTermRow['latest_term']) ? (int)$latestTermRow['latest_term'] : null;
$termIsLatest = ($filters['term'] !== null && $latestTermInSlice !== null && (int)$filters['term'] === (int)$latestTermInSlice);
...
if ($ajax === 'pupil_scores') {
    ...
    // If user chose the latest term, show ALL terms in the modal (but still highlight latest vs previous).
    if ($filters['term'] && !$termIsLatest) { $w[] = 'e.term = :t'; $pp['t'] = (int)$filters['term']; }
    ...
    if ($filters['exam_id']) {
        ...
        $pair = fetch_all($pdo, "
            ...
            FROM results r
            JOIN pupils p2  ON p2.id = r.pupil_id
            JOIN subjects s2 ON s2.id = r.subject_id
            JOIN exams e    ON e.id = r.exam_id
            $whereNoExamSql
            GROUP BY e.id, e.exam_date
            ORDER BY COALESCE(e.exam_date,'9999-12-31') DESC, e.id DESC
            LIMIT 2
        ", $paramsNoExam);

        if (count($pair) === 2 && (int)$pair[0]['id'] === $pinnedExamId) {
            $prevId = (int)$pair[1]['id'];
            $w[] = '(e.id = :eid OR e.id = :pid)';
            $pp['eid'] = $pinnedExamId;
            $pp['pid'] = $prevId;
        } else {
            $w[] = 'e.id = :eid';
...
if ($ajax === 'subject_trend') {
    ...
    if ($filters['academic_year']) { $w[] = 'e.academic_year = :ay'; $pp['ay'] = $filters['academic_year']; }
    // If user chose the latest term, show ALL terms in the modal.
    if ($filters['term'] && !$termIsLatest) { $w[] = 'e.term = :t'; $pp['t'] = (int)$filters['term']; }
...
/* ----------------------------- subject delta (latest vs previous) ----------------------------- */

$subjectDeltaMap = []; // subject_id => ['delta'=>float, 'pct'=>?float]
$subjectExamPair = null; // [latestId, prevId] for exam-to-exam
$subjectTermPair = null; // [latestTerm, prevTerm] for term-to-term

if ($nResults > 0 && !$filters['exam_id']) {
    // If user chose the latest term, compare TERM-to-TERM (latest term vs previous term).
    if ($termIsLatest && $filters['term'] !== null && (int)$filters['term'] > 1) {
        $tLatest = (int)$filters['term'];
        $tPrev   = $tLatest - 1;
        $subjectTermPair = [$tLatest, $tPrev];
        ...
        ", array_merge($paramsNoExamNoTerm, ['t' => $tLatest]));
        ...
        $m2 = fetch_all($pdo, "
            ...
            AND e.term = :t
            GROUP BY r.subject_id
        ", array_merge($paramsNoExamNoTerm, ['t' => $tPrev]));
        ...
        // Otherwise compare the last two exams inside THIS slice (ignoring exam_id).
        $pair = fetch_all($pdo, "
            SELECT
              e.id,
              e.exam_date,
...
// Class/Track delta (latest vs previous) for comparison table.
// Keyed by "class_code|track" => ['mean_delta'=>..., 'mean_pct'=>..., 'pass_delta'=>..., 'pass_pct'=>...]
$classDeltaMap = [];
$classDeltaBasis = null; // 'term' or 'exam'
if ($nResults > 0 && !$filters['exam_id']) {
    if ($termIsLatest && $filters['term'] !== null && (int)$filters['term'] > 1) {
        $classDeltaBasis = 'term';
        $tLatest = (int)$filters['term'];
        $tPrev   = $tLatest - 1;

        $aRows = fetch_all($pdo, "
            SELECT p.class_code, p.track, AVG(r.score) AS mean_score, AVG(r.score >= :pass) * 100.0 AS pass_rate
            FROM results r
            JOIN pupils p   ON p.id = r.pupil_id
            JOIN subjects s ON s.id = r.subject_id
            JOIN exams e    ON e.id = r.exam_id
            ...
            AND e.term = :t
            GROUP BY p.class_code, p.track
        ", array_merge($paramsNoExamNoTerm, ['t' => $tLatest, 'pass' => $passThreshold]));
        ...
        $bRows = fetch_all($pdo, "
            ...
            AND e.term = :t
            GROUP BY p.class_code, p.track
        ", array_merge($paramsNoExamNoTerm, ['t' => $tPrev, 'pass' => $passThreshold]));

        $a = [];
        foreach ($aRows as $r) $a[(string)$r['class_code'] . '|' . (string)$r['track']] = ['mean' => (float)$r['mean_score'], 'pass' => (float)$r['pass_rate']];
        $b = [];
        foreach ($bRows as $r) $b[(string)$r['class_code'] . '|' . (string)$r['track']] = ['mean' => (float)$r['mean_score'], 'pass' => (float)$r['pass_rate']];

        $keys = array_unique(array_merge(array_keys($a), array_keys($b)));
        foreach ($keys as $k) {
            if (!isset($a[$k], $b[$k])) continue;
            $dm = $a[$k]['mean'] - $b[$k]['mean'];
            $dp = (abs($b[$k]['mean']) > 0.00001) ? ($dm / $b[$k]['mean']) * 100.0 : null;
            $dpr = $a[$k]['pass'] - $b[$k]['pass'];
            $dprp = (abs($b[$k]['pass']) > 0.00001) ? ($dpr / $b[$k]['pass']) * 100.0 : null;
            $classDeltaMap[$k] = ['mean_delta' => $dm, 'mean_pct' => $dp, 'pass_delta' => $dpr, 'pass_pct' => $dprp];
        }
    } else {
        $classDeltaBasis = 'exam';
        // Find the last two exams present in THIS slice (ignoring exam_id).
        $pair = fetch_all($pdo, "
            SELECT e.id, e.exam_date, e.exam_name
            FROM results r
            JOIN pupils p   ON p.id = r.pupil_id
            JOIN subjects s ON s.id = r.subject_id
            JOIN exams e    ON e.id = r.exam_id
        ...
        if (count($pair) === 2) {
            ...
            $aRows = fetch_all($pdo, "
                ...
                AND e.id = :eid
                GROUP BY p.class_code, p.track
            ", array_merge($paramsNoExam, ['eid' => $latestId, 'pass' => $passThreshold]));
            ...
            $bRows = fetch_all($pdo, "
                ...
                AND e.id = :eid
                GROUP BY p.class_code, p.track
            ", array_merge($paramsNoExam, ['eid' => $prevId, 'pass' => $passThreshold]));
      ...
      <div class="card-body">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
          ...
          <div class="small text-muted">
            Click a subject to open an all-terms view and see changes (points & %).
              ...
              <tbody>
                <?php foreach ($classSummary as $i => $r): ?>
                  <?php
                    ...
                    $k = (string)$r['class_code'] . '|' . (string)$r['track'];
                    $dm = $classDeltaMap[$k]['mean_delta'] ?? null;
                    $dmp = $classDeltaMap[$k]['mean_pct'] ?? null;
                    $dpass = $classDeltaMap[$k]['pass_delta'] ?? null;
                    $dpassp = $classDeltaMap[$k]['pass_pct'] ?? null;
                    $dmMeta = ($dm !== null) ? delta_badge((float)$dm) : null;
                    $dpMeta = ($dpass !== null) ? delta_badge((float)$dpass) : null;

                    // Rank badge styling for places 1-3
                    if ($rank === 1) { $rankCls = 'text-bg-warning text-dark'; $rankIc = 'bi-trophy'; }
                    elseif ($rank === 2) { $rankCls = 'text-bg-secondary'; $rankIc = 'bi-award'; }
                    elseif ($rank === 3) { $rankCls = 'text-bg-danger'; $rankIc = 'bi-award'; }
                    else { $rankCls = 'text-bg-dark'; $rankIc = 'bi-hash'; }
                  ?>
                  <tr>
                    <td class="text-center">
                      <span class="badge <?= eh($rankCls) ?> rank-badge"><i class="bi <?= eh($rankIc) ?> me-1"></i><?= $rank ?></span>
                    </td>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div id="pupilScoresContent" class="d-none">
          ...
          <div class="small text-muted mt-2">
            If Term filter is set to the latest term, the modal shows all terms (to enable comparison). Otherwise it shows the selected term only.
          </div>
        </div>
      </div>
...
(() => {
  ...
  const pkLastDeltaSub = document.getElementById('pk_last_delta_sub');
  ...
  async function loadPupilScores(pupilId, label) {
    ...
    try {
      ...
      const pinnedExamId = data.pinned_exam_id ? String(data.pinned_exam_id) : null;
      ...
      if (ins.lastDeltaPts !== null && Number.isFinite(ins.lastDeltaPts)) {
        ...
        let lastDeltaPct = null;
        if (ins.examsChrono && ins.examsChrono.length >= 2) {
          const a = ins.examsChrono[ins.examsChrono.length - 1];
          const b = ins.examsChrono[ins.examsChrono.length - 2];
          const bTot = Number(b.total);
          if (Number.isFinite(bTot) && Math.abs(bTot) > 0.00001) {
            lastDeltaPct = ((Number(a.total) - bTot) / bTot) * 100;
          }
        }
      ...
      for (const ex of ins.examsChrono) {
        ...
        if (pinnedExamId && String(ex.exam_id) === pinnedExamId) tr.classList.add('table-primary');
      ...
      for (const r of rows) {
        const tr = document.createElement('tr');
        if (pinnedExamId && String(r.exam_id) === pinnedExamId) tr.classList.add('table-primary');
        if (pinnedExamId && String(r.exam_id) === pinnedExamId) tr.classList.add('table-primary');
